...
import { getDb, getProjectByPath, ensureDataDir, } from '../../db/index.js';
...
function isValidExtraction(sentence) {
    if (sentence.length < 30 || sentence.length > 500)
        return false;
    if ((sentence.match(/{/g) || []).length + (sentence.match(/}/g) || []).length > 3)
        return false;
...
function findClaudeProject(projectPath) {
...
function parseClaudeConversationFile(filePath) {
...
async function importFromClaude(projectPath, projectId, existingExtractions, dryRun, limit, full = false) {
...
// Aider import
// ============================================================================
function findAiderHistoryFiles(projectPath) {
    const files = [];
