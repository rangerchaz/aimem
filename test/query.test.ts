...
import { formatContextForInjection, type ContextResult } from '../src/query/index.js';
...
describe('Query Engine', () => {
  ...
  describe('Conversation Search', () => {
    it('should find conversations by content', () => {
      ...
      createTestConversation(
        testDb.db,
        project.id,
        'We discussed implementing a caching layer using Redis for better performance.',
        'Redis caching discussion'
      );

      createTestConversation(
        testDb.db,
        project.id,
        'Set up the PostgreSQL database with proper indexes.',
        'PostgreSQL setup'
      );
    ...
    it('should search by summary', () => {
      ...
      createTestConversation(
        testDb.db,
        project.id,
        'Long conversation content here...',
  ...
  describe('Linked Context', () => {
    it('should retrieve extractions linked to structures', () => {
      ...
      createTestExtraction(
        ...
        conv.id,
        'decision',
        'We should validate all inputs in handleRequest before processing'
      );
      ...
      `).all('structure', structure.id) as any[];
  ...
  describe('Project Scoping', () => {
    it('should scope structure search by project', () => {
      ...
      `).all('sharedFunc', project1.id) as any[];
