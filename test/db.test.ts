...
describe('Database Operations', () => {
  ...
  describe('Projects', () => {
    ...
    it('should query projects by path', () => {
      ...
      const result = testDb.db.prepare('SELECT * FROM projects WHERE path = ?').get('/unique/path') as any;
  ...
  describe('Structures', () => {
    it('should create a structure', () => {
      ...
      const structure = createTestStructure(
        ...
        'function processData(input: string) { return input; }'
      ) as any;
  ...
  describe('Extractions', () => {
    it('should create an extraction', () => {
      ...
      const extraction = createTestExtraction(testDb.db, conv.id, 'decision', 'We should use TypeScript') as any;
    ...
    it('should cascade delete extractions when conversation is deleted', () => {
      ...
      const conv = createTestConversation(testDb.db, project.id, 'Content');
      const extraction = createTestExtraction(testDb.db, conv.id, 'decision', 'A decision') as any;
  ...
  describe('Links', () => {
    it('should create a link between entities', () => {
      ...
      const structure = createTestStructure(testDb.db, file.id, 'function', 'linked', 1, 5, null, 'code') as any;
      const conv = createTestConversation(testDb.db, project.id, 'Conversation about linked');

      const link = createTestLink(testDb.db, 'conversation', conv.id, 'structure', structure.id, 'decision') as any;
    ...
    it('should enforce unique links', () => {
      ...
      const structure = createTestStructure(testDb.db, file.id, 'class', 'UniqueClass', 1, 10, null, 'class') as any;
      const conv = createTestConversation(testDb.db, project.id, 'Content');
    ...
    it('should query links by source', () => {
      ...
      const s1 = createTestStructure(testDb.db, file.id, 'function', 'func1', 1, 5, null, 'code') as any;
      const s2 = createTestStructure(testDb.db, file.id, 'function', 'func2', 6, 10, null, 'code') as any;
      ...
      ).all('conversation', conv.id) as any[];
