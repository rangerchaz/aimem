...
function getPlatform(): 'linux' | 'macos' | 'windows' | 'wsl' {
...
async function installCertificate(certPath: string, platform: string): Promise<boolean> {
...
function setupAutostart(aimemPath: string, port: string, platform: string): boolean {
...
function addToShellProfile(profilePath: string, port: string, certPath: string | null): boolean {
...
async function setupCursor(options: { port?: string }): Promise<void> {
  const port = options.port || '8080';
